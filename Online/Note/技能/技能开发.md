...
pParam->GetCaster()//CanCastParam-CRole
GetHurter()//伤害来源
GetTarget()//受伤角色
pPlayCard->GetOwner()
pUseCardAction->GetCardUser();
pZoneBase->GetPlayCardOwner();
pSpell->GetCaster()
GetCurTargetRole();
...
CSpellMgr::single()->EffectCanMoveCardToEquipZone(*GetGame(), *m_pRoleTarget[i], pCard->GetSpellId(), CardIdList{ pCard->GetCardId() })//能否移动到某区域
CSpellMgr::single()->CanUseCardByBaseCondition(pGame, param) != SPELL_CAST_OK//是否具备使用条件
pCard = CSpellMgr::single()->GetCardUseLikeMyCard(*pParam->psrc_role, pParam->use_cardlist[i], (unsigned int)YOKA_SKILL::SKILL_SHA, (unsigned int)YOKA_SKILL::SKILL_CHARACTER_JIE_FU_HUN);//常用于转化牌检查
```
...
GetCaster()->GetSpellStateMgr()->RemoveSpellStateBySpellId((unsigned int)YOKA_SKILL::SKILL_ROGUELIKE_BUFF_WULIE);
...
IsCurrentPhase(PHASE_END)//处于指定阶段内
GetDealCardCnt()//最终要摸牌的数量
```
### CCardDataRepository
```cpp
CCardDataRepository::Singleton(pGame->GetGameStartTime())->GetCardTypeBySpellId(pSpell->GetSpellId()) != CARD_TYPE_NORMAL//牌类型
eCardSubType cardSubType = CCardDataRepository::Singleton(pGame->GetGameStartTime())->GetCardSubTypeBySpellId(pAc->GetUseSpellId());//牌子类型
CCardDataRepository::Singleton(pGame->GetGameStartTime())->IsNormalStratagem(pSpell->GetSpellId())//是否为普通锦囊类型
...
### CPlayCard
```cpp
CardIdList cardSuitList;//uint列表,一般存储牌id,此处只存储花色0-4
for (CardIdList::const_iterator iter = pParam->use_cardlist.begin(); iter != pParam->use_cardlist.end(); ++iter)//遍历CardIdList
{
    CPlayCard* pPlayCard = pParam->GetCaster()->GetHandCardZone()->Find(*iter);
    if (NULL == pPlayCard)
        continue;
    //CPlayCard::GetColorBySuit(pPlayCard->GetSuit())//花色判断颜色
    cardSuitList.push_back(pPlayCard->GetSuit());
}
CPlayCard::GetPlayCardColorBySuitList(cardSuitList);//花色列表判断颜色


- TO3
GetZone()//-CZoneBase
GetOwner()//-CRole
PPlayCardList//-CPlayCard列表

- 3TO
pZoneBase->GetPlayCardList()//PPlayCardList&
pZoneBase->->Find(*iter)
pZoneBase->Random();
pRole->GetMyPlayCard(pMsg->data[0]);
pRole->RandomGetMyCard();
pRole->RandomGetMyCanDisCard(GetSpellId());
pUseCardAction->GetPlayCards();
```
### CZoneBase
```cpp
GetZoneType()!=ZONE_DISACARPILE
GetPlayCardList()//PPlayCardList&
GetPlayCardOwner()Zon//-CRole
Find(*iter)
Random();
...
GetTarget()//受伤角色
GetSrcAction()
GetReceiveDamageTimes()//当前回合内受到伤害的次数
AddDamageHp(1);
PreventDamage();
...
CMoveCardAction* pAc = CMoveCardAction::MoveCards(GetGame(), EmActMode::Mode_Get,pTarget->GetHandCardZone()->GetPlayCardList(), seatIdList,GetCaster()->GetSeatId(), GetCaster(), GetSpellId(),pTarget->GetHandCardZone(), GetCaster()->GetHandCardZone(),pTarget->GetSeatId(), GetCaster()->GetSeatId());

CMoveCardAction::MoveCardsFromRole(pGame, EmActMode::Mode_DisCard, cards, seats, pCaster->GetSeatId(), pCaster, GetSpellId(), pCaster->GetSeatId(),&pGame->GetDiscardZone(),sc_invalid_seat_id);//

CMoveCardAction::MoveCardsEx(GetGame(), EmActMode::Mode_Get, PPlayCardList{ showcard },GetGame()->GetRoleList(GetCasterSeatId(), false, false), GetCasterSeatId(), GetCaster(), GetSpellId(),GetCaster()->GetHandCardZone());
...
```cpp
GetCardUser();//-CRole
GetPlayCards();//PPlayCardList&
GetUseCardNumber();//点数
GetUseSpellId()
GetUseCardRecordIndex()//使用牌次数
GetUseMode()//使用/打出
GetSubstituteSpell()//技能使用牌
GetSrcRole()
GetRoleCardFor()//替使用
- 3TO
...
GetCurTarget()
pSpell->GetTargetList()
GetTarget_Opt()//技能牌的目标

GetUsedCardSuit()
GetUsedCardAction()//-CUseCardAction
...
SetAllTargetDirectHurtState()//不可闪避
- TO3
GetUsedCardAction()//-CUseCardAction
- 3TO
...
- 3TO
const CPlayCardData* playCardData = CCardDataRepository::Singleton(GetGame()->GetGameStartTime())->GetPlayCardData(uCardId);
```
...
DisCardFromZone(GetCaster(), pZoneBase, playCardList);
```

### IN
```cpp
//肉鸽
pGame->IsEnemy(*pRole, *pCurRole)
auto pCfg = pGame->GetTacticsNodeFromSpellId(Spell_Id);
```
...
```cpp
//.h
const unsigned int Template_InitId_YaoLiJingTong = (unsigned int)YOKA_SKILL::INVALID_SPELL_ID;
//Spell_Id模板，在类前声明
template<unsigned int Spell_Id = Template_InitId_YaoLiJingTong>
//CreateXSpell填充模板类和模板id
class Roguelike_YaoLiJingTong : public CSpell, public CreateXSpell<Roguelike_YaoLiJingTong<Spell_Id>, Spell_Id>
{
public:
};
//data同上，在类前声明
template<unsigned int Spell_Id = Template_InitId_YaoLiJingTong>
class Roguelike_YaoLiJingTongData :public CRoleSpellData {
public:
};

//.cpp
const unsigned int ID_RTS_SKILL_ROGUELIKE_YAO_LI_JING_TONG6 = (unsigned int)YOKA_SKILL::SKILL_ROGUELIKE_YAO_LI_JING_TONG6;
REGISTER_TRIGGER_SPELL(Roguelike_YaoLiJingTong<ID_RTS_SKILL_ROGUELIKE_YAO_LI_JING_TONG6>, ID_RTS_SKILL_ROGUELIKE_YAO_LI_JING_TONG6)
//Spell_Id模板，在每个函数前声明
template<unsigned int Spell_Id /*=  (unsigned int)YOKA_SKILL::INVALID_SPELL_ID*/> 
//Spell_Id在每个类名后填充
Roguelike_YaoLiJingTong<Spell_Id>::Roguelike_YaoLiJingTong(const CCardSpellData* pSpellData, CGame* pGame) : CSpell(pSpellData, pGame, 0) 
{
}

//data同上，在每个函数前声明，在每个类名后填充
REGISTER_SPELL_DATA_CREATE(ID_RTS_SKILL_ROGUELIKE_YAO_LI_JING_TONG6, Roguelike_YaoLiJingTongData<ID_RTS_SKILL_ROGUELIKE_YAO_LI_JING_TONG6>)

template<unsigned int Spell_Id /*=  (unsigned int)YOKA_SKILL::INVALID_SPELL_ID*/>
Roguelike_YaoLiJingTongData<Spell_Id>::Roguelike_YaoLiJingTongData(CRole* pRole) : CRoleSpellData((unsigned int)Spell_Id, pRole)
{
	m_reset_type = reset_die | reset_turn_end;
}
```
...
## 上传
- mod
[OLH5-81545]士气剥夺更改为游戏开始时机

- mod
[OLH5-81546]粮草剥夺检查下限：当前摸牌数大于2，则最多减至2

- mod
[OLH5-81118]DBS优化指针判空检查

- mod
[OLH5-79823,OLH5-79825]肉鸽技能组返回值优化；除疠，肉鸽除疠超时弃置鸡肋牌问题修复

- mod
[mod][OLH5-79304]前端要求更改父魂颜色枚举值-闪
...
- CONST
- REGISTER
    - `cardIdDefineEnumClass.h`枚举技能SKILL_ROGUELIKE_PO_JUN = 14112
    - 注册技能类型`REGISTER_TRIGGER_SPELL`
